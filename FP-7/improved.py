...
def make_pipe_improved(feature_cols, alpha=10.0, random_state=42):
...
def cv_rmse_improved(df_sub, feature_cols, n_splits=5, random_state=42):
    ...
    pipe = make_pipe_improved(feature_cols, alpha=10.0, random_state=random_state)
...
def compute_delta_rmse_by_tier_improved(df, random_state=42):
    ...
    for tier_name, tier_df in tiers.items():
        ...
        rmse_base, used_base = cv_rmse_improved(tier_df, BASELINE, random_state=random_state)
        rmse_mmi, used_mmi = cv_rmse_improved(tier_df, PLUS_MMI, random_state=random_state)
...
def plot_main_result_1_delta_rmse_improved():
    delta_df = _IMPROVED_OUT.get("delta_df")
    if delta_df is None or len(delta_df) == 0:
        raise ValueError("delta_df is empty. Run run_all_improved_analyses() first.")
...
def compute_error_by_mmi_improved(
    df,
    random_state=42,
    min_group_n=8,
    min_rows_total=25,
    ...
    pipe = make_pipe_improved(best_features, alpha=10.0, random_state=random_state)
...
def plot_main_result_2_error_by_mmi_improved():
    plot_df = _IMPROVED_OUT.get("err_plot_df")
    test_metrics = _IMPROVED_OUT.get("test_metrics")
    meta = _IMPROVED_OUT.get("error_meta")

    if plot_df is None or test_metrics is None or meta is None:
        raise ValueError("Missing stored results. Run run_all_improved_analyses() first.")
...
def plot_learning_curve_extra_improved():
    df = _IMPROVED_OUT.get("df")
    feats = _IMPROVED_OUT.get("FULL_features")
    pipe = _IMPROVED_OUT.get("full_pipe")

    if df is None or feats is None or pipe is None:
        raise ValueError("Run run_all_improved_analyses() first.")
...
def run_all_improved_analyses(path="Earthquakes.csv", random_state=42):
    df = load_earthquakes(path)
    _IMPROVED_OUT["df"] = df

    delta_df = compute_delta_rmse_by_tier_improved(df, random_state=random_state)
    _IMPROVED_OUT["delta_df"] = delta_df

    err_plot_df, test_metrics, feats, pipe, meta = compute_error_by_mmi_improved(
        df,
        random_state=random_state,
        min_group_n=8,
        min_rows_total=25,
    ...
    _IMPROVED_OUT["err_plot_df"] = err_plot_df
    _IMPROVED_OUT["test_metrics"] = test_metrics
    _IMPROVED_OUT["FULL_features"] = feats
    _IMPROVED_OUT["full_pipe"] = pipe
    _IMPROVED_OUT["error_meta"] = meta


def improved_outputs_keys():
    return list(_IMPROVED_OUT.keys())


run_all_improved_analyses("Earthquakes.csv")
plot_main_result_1_delta_rmse_improved()
plot_main_result_2_error_by_mmi_improved()
plot_learning_curve_extra_improved()
